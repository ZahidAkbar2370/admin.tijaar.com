...
<div class="w-full min-w-0">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Edit: Terms of Service</h1>
        <p class="text-gray-500 mt-1">Update the Terms page: hero, last updated date, numbered sections (table below), and footer. Use the table to add, edit, delete, or reorder sections. Content uses a rich text editor.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.pages.update', $page) }}" id="terms-form" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $page->title) }}" required placeholder="Terms of Service" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Hero title</label>
                    <input type="text" name="banner_title" value="{{ old('banner_title', $page->banner_title ?? 'Terms of Service') }}" placeholder="Terms of Service" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Hero description (subtitle)</label>
                    <input type="text" name="banner_subtitle" value="{{ old('banner_subtitle', $page->banner_subtitle ?? 'Please read these terms carefully. By using Tijaar, you agree to be bound by the following conditions.') }}" placeholder="Short line under the hero title" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                </div>
                <div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="footer_contact_text" value="{{ old('footer_contact_text', $footerContact) }}" placeholder="If you have any questions about these Terms, please contact us at" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm" />
                </div>
                <div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ url('terms') }}" target="_blank" rel="noopener" class="ml-auto text-sm text-primary hover:underline">View Terms page on site →</a>
        </div>
    </form>
</div>
