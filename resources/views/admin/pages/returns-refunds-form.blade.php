...
@section('title', 'Edit: Returns & Refunds')
...
<div class="w-full min-w-0">
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Edit: Returns & Refunds</h1>
        <p class="text-gray-500 mt-1">Update the Returns & Refunds page: hero, last updated date, numbered sections (table below), and footer. Use the table to add, edit, delete, or reorder sections. Content uses a rich text editor.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.pages.update', $page) }}" id="returns-refunds-form" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-primary/5 to-transparent">
                ...
                <p class="text-sm text-gray-500 mt-1">Title and the header at the top. Last updated appears under the title.</p>
            </div>
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Page title <span class="text-red-500">*</span></label>
                    <input type="text" name="title" value="{{ old('title', $page->title) }}" required placeholder="Returns & Refunds" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Hero title</label>
                    <input type="text" name="banner_title" value="{{ old('banner_title', $page->banner_title ?? 'Returns & Refunds') }}" placeholder="Returns & Refunds" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Last updated (shown under title)</label>
                    <input type="text" name="last_updated" value="{{ old('last_updated', $sections['last_updated'] ?? '') }}" placeholder="e.g. March 2024" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary" />
                </div>
            </div>
        </div>
        ...
        @include('admin.pages.partials.sections-table', [
            'page' => $page,
            'sections' => $sections,
            'prefix' => 'returns-refunds',
            'sectionsTitle' => 'Returns & Refunds sections',
        ])
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="footer_contact_text" value="{{ old('footer_contact_text', $footerContact) }}" placeholder="Questions about returns or refunds? Contact us." class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm" />
                </div>
                <div>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="{{ url('returns-refunds') }}" target="_blank" rel="noopener" class="ml-auto text-sm text-primary hover:underline">View Returns & Refunds page on site</a>
        </div>
    </form>
</div>

@include('admin.pages.partials.section-modal', ['page' => $page, 'prefix' => 'returns-refunds'])
@endsection
